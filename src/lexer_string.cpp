...
namespace rattle::lexer {
  static char _hex2char(char hex) {
    return hex - (isdec(hex) ? '0' : (std::islower(hex) ? 'a' : 'A'));
  }
  static void consume_string_escape(State &state, token::Token::Kind &kind) {
  ...
  static token::Token consume_string_helper(State &state) {
